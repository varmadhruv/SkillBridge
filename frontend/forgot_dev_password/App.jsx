...
function App() {
  ...
  const [devId, setDeveloperId] = useState('');
  ...
  const handleSendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:5000/forgot-dev-password', {
        method: 'POST',
  ...
  const handleResetPassword = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:5000/reset-dev-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ devId, newPassword: newPassword.trim() })
      });
      const data = await res.json();
      if (res.ok) {
        toast.success('Password reset successfully! Redirecting...');
        setTimeout(() => { window.location.href = '/Developer/Login/index.html'; }, 1500);
      } else {
    ...
    <div className="fp-page">
      ...
      <div className="fp-card">
        ...
        <p className="fp-subtitle">
          {!emailSubmitted
            ? 'Enter your registered dev email to receive an OTP'
            : !otpVerified
        ...
        <button className="fp-back-link" onClick={() => window.location.href = '/Developer/Login/index.html'}>
