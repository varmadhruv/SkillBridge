...
function App() {
  ...
  const handleSendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:5000/forgot-admin-password', {
        method: 'POST',
  ...
  const handleResetPassword = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch('http://127.0.0.1:5000/reset-admin-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminId, newPassword: newPassword.trim() })
      });
      const data = await res.json();
      if (res.ok) {
        toast.success('Password reset successfully! Redirecting...');
        setTimeout(() => { window.location.href = '/Admin/Login/index.html'; }, 1500);
      } else {
  ...
  return (
    <div className="fp-page">
      ...
      <div className="fp-card">
        ...
        <button className="fp-back-link" onClick={() => window.location.href = '/Admin/Login/index.html'}>
          ← Back to Admin Login
        </button>
      </div>
    </div>
