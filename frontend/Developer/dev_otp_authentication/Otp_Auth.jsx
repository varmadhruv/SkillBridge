...
function Otp_Auth() {
  ...
  useEffect(() => {
    ...
    if (!isMounted.current) {
      // Check if OTP already exists, if not, resend might be needed 
      // but usually login endpoint sends it first.
      // We only auto-resend if there's no tempOtp in storage.
  ...
  return (
    <div className="validation-container">
      ...
      <p style={{ color: '#666', fontSize: '14px', marginBottom: '25px', textAlign: 'center' }}>
        Hello {developerName}, an OTP has been sent to your registered email.
      </p>
      ...
      <button className="back-btn" onClick={() => window.location.href = '/Developer/Login/index.html'}>Back</button>
    </div>
  );
}
