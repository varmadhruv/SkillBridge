...
app.post("/resend-otp", async (request, response) => {
      try {
        ...
        try {
          await sendEmail(
            email,
            "SkillBridge Login OTP - Resend",
            `
      <h2>Your OTP for SkillBridge Login</h2>
      <p>Hello,</p>
...
app.post("/create-zoom-meeting", async (req, res) => {
      try {
        ...
        if (mentor && mentor.email) {
          try {
            await sendEmail(
              mentor.email,
              `Payment Received - Session with ${studentName}`,
              `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border-radius: 15px; background: linear-gradient(135deg, #fdfcfb 0%, #e2d1c3 100%); border: 1px solid #ddd; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        ...
        <p style="font-size: 1.1rem; color: #333; line-height: 1.6;">
          <strong>Good news!</strong> The student
          <strong>${studentName}</strong> has successfully completed the payment
          and is currently waiting for you to join and start the session.
        </p>
        ...
        <p style="font-size: 0.8rem; color: #777; text-align: center;">
          This is an automated request from SkillBridge Mentorship Platform.
        </p>
      </div>
      `
            );

            console.log("BREVO SUCCESS: Payment email sent to " + mentor.email);
          } catch (mailError) {
...
app.post("/admin-login", async (request, response) => {
      ...
      <p>An attempt to login to the admin panel was made.</p>
...
app.post("/admin-otp-resend", async (request, response) => {
      try {
        ...
        await sendEmail(
          ...
          "Admin Login OTP",
          `
    <h2>Your OTP for Admin Verification</h2>
    <p>Hello ${admin.AdminName},</p>
    <p>Your 6-digit OTP for admin verification is:
...
app.post("/booking-request", async (req, res) => {
      try {
        ...
        try {
          await sendEmail(
            mentor.email,
            `New Mentorship Request - ${studentName}`,
            `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; padding: 30px; border-radius: 15px; background: linear-gradient(135deg, #fdfcfb 0%, #e2d1c3 100%); border: 1px solid #ddd; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        <h2 style="color: #7b2cbf; margin-bottom: 20px;">Hello ${mentor.fullName},</h2>
        ...
        <p style="font-size: 1.1rem; color: #333; line-height: 1.6;">
          ...
          Please log in to your dashboard and navigate to the
          <strong>Updates</strong> section. Mark yourself as
          <strong>"Available"</strong> to confirm the request and start the session.
        </p>
        ...
        <div style="text-align: center;">
          <a href="${FRONTEND_URL}/Mentor_home_page/"
             style="display: inline-block; padding: 15px 30px; background: #7b2cbf; color: white; text-decoration: none; border-radius: 50px; font-weight: bold; box-shadow: 0 5px 15px rgba(123,44,191,0.3);">
    ...
    `
          ...
          console.log("BREVO SUCCESS: Email sent to " + mentor.email);
        } catch (mailError) {
          console.error("BREVO ERROR:", mailError);
        }
...
app.post("/developer-forgot-otp", async (request, response) => {
      try {
        ...
        await sendEmail(
          developer.email,
          "Developer Password Recovery OTP",
          `
        <h2>Your OTP for Password Recovery</h2>
        <p>Hello ${developer.fullName},</p>
        <p>Your 6-digit OTP for password recovery is:
          <strong>${generatedOtp}</strong>
        </p>
      `
        );
      ...
      try {
        ...
        try {
          await sendEmail(
            ...
            "Developer Login OTP",
            `
      <h2>Your OTP for Developer Panel Login</h2>
      <p>Hello ${developer.fullName},</p>
      <p>An attempt to login to the Developer panel was made.</p>
      <p>Your 6-digit OTP is: <strong>${generatedOtp}</strong></p>
    `
          );
...
app.post("/forgot-mentor-password", async (request, response) => {
    ...
    <p>Your 6-digit OTP to reset your password is:
      <strong>${otp}</strong>
    </p>
...
app.post("/forgot-admin-password", async (request, response) => {
          try {
            ...
            await sendEmail(
              admin.AdminEmail,
              "SkillBridge Admin - Password Reset OTP",
              `
    <h2>Admin Password Reset Request</h2>
    <p>Hello ${admin.AdminName},</p>
    <p>Your 6-digit OTP to reset your admin password is:
      <strong>${otp}</strong>
    </p>
    <p>This OTP is valid for one-time use only.</p>
  `
...
app.post("/forgot-dev-password", async (request, response) => {
              try {
                ...
                try {
                  await sendEmail(
                    developer.email,
                    "SkillBridge Developer - Password Reset OTP",
                    `
      <h2>Developer Password Reset Request</h2>
      <p>Hello ${developer.fullName},</p>
      <p>Your 6-digit OTP to reset your developer password is:
        <strong>${otp}</strong>
      </p>
      <p>This OTP is valid for one-time use only.</p>
    `
                  ...
                  console.log("Developer password reset OTP sent to:", developer.email);
                } catch (mailError) {
                  console.error("Failed to send developer reset OTP:", mailError);
                  return response.status(500).json({ message: "Failed to send OTP" });
                }
